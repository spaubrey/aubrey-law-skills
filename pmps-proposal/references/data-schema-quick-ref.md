...
## plan.planStructure (added June 2026)

Master switch controlling four estate-plan structures. When present, the
templates derive branching from it via `shared.resolvePlanStructure(data)`;
when absent, they fall back to the legacy top-level booleans (backward
compatible).

| Value | clients.type | hasCreditShelter | clayton | maQtip | Trusts |
|---|---|---|---|---|---|
| `joint_outright` | couple | false | false | false | 1 joint |
| `joint_disclaimer` | couple | true | false | true | 1 joint (disclaimer) |
| `separate_clayton` | couple | true | true | true | 2 (one per spouse) |
| `individual_single` | individual | false | false | false | 1 individual |

Companion fields on `plan`: `clayton` (true only for separate_clayton),
`maQtip` (true for joint_disclaimer and separate_clayton).

Documents may carry `forSpouse: 1|2` (separate_clayton's per-spouse trusts) in
addition to `joint` / `perSpouse` / `displayName` / `description`.

`tax.scenario2` is null for `joint_outright` and `individual_single`. For the
two $4M structures it is nested: `{firstDeath:{bypassAmount,netTax},
secondDeath:{taxableEstate,...,netTax}, totalNetTax}`, where
`secondDeath.taxableEstate = grossEstate − 4,000,000`.

`considerations.businessInterest` (or `outOfStateProperty`) triggers the §35
out-of-state-property note and the ILIT-liquidity bullet.

All `netTax` figures are net of the $99,600 unified credit. See
`references/ma-estate-tax-computation.md`. ALL figures are estimates pending
M-706 verification.

> Full behavior matrix: `references/pmps-planstructure-mapping.md`.
