...
## June 2026 — Four plan structures + Clayton/QTIP + unified proposal

- Added `plan.planStructure` (`joint_outright` | `joint_disclaimer` |
  `separate_clayton` | `individual_single`) as the master switch, resolved via
  `shared.resolvePlanStructure(data)`. Backward compatible: falls back to the
  legacy booleans when `planStructure` is absent. All four Templates (agenda,
  tax-worksheet, proposal, one-pager) branch on it. See
  `references/pmps-planstructure-mapping.md`.

- **Proposal + one-pager replaced with the upgraded standalone `pmps-proposal`
  skill versions**, so the 4-doc PMPS package and the standalone 2-doc skill now
  produce IDENTICAL proposal/one-pager output. This brought in the richer
  Executive Summary, the "What This Plan Accomplishes" bullets, the structure-
  specific "How the Credit Shelter Planning Works" (Clayton/QTIP/Shaffer) section,
  and §35 notes.

- **`_shared.js` unified.** pmps-prep now uses the standalone skill's `_shared.js`
  (a superset that exports everything the agenda + tax-worksheet need — dataCell,
  hdrCell, flagCell, totalCell, color tokens — plus the proposal helpers
  makeHeaderProposal, clientIdentifier, subSectionHdr, etc.). One shared helper
  file across both skills.

- **Net tax authoritative** (Scott, June 2026). Templates display the pre-supplied
  `tax` object; figures are net of the $99,600 unified credit. **$4M fix**: the
  both-exemptions scenario shelters gross − $4,000,000. **Anti-cliff credit**:
  references corrected from pre-2023 "entire estate taxed" to post-2023 credit.

- Agenda + tax-worksheet gained structure-specific content: disclaimer-dependency
  language, Clayton/MA-QTIP/Shaffer notes, §35 situs review, joint_outright
  lost-exemption framing. Fiduciary keys normalized (accepts trustee/initialTrustee
  and successorTrustee/successorTrustees[]).

- `references/` added (ma-estate-tax-computation.md corrected; pmps-planstructure-
  mapping.md new). `Templates/pmps-data-schema.js` updated with four SAMPLE objects
  (SAMPLES map) + updated EMPTY + planStructure/clayton/maQtip.

**ALL tax figures are estimates pending Scott's M-706 verification.**

---

## June 2026 (follow-up) — Tax calculator added (root-cause fix for gross/net)

Root cause of the Venables GROSS output identified: there was never a tax
calculator — Cowork hand-computed brackets per matter following SKILL.md, and a
gross figure was written where net belonged. Added `Templates/pmps-tax-calc.js`
exporting `computeMATax(grossEstate, planStructure)`, which returns the fully
populated, NET, four-structure-correct `tax` object ($4M shelter where
applicable). SKILL.md Step 3 now instructs calling the calculator instead of
hand-computing. Validated: calculator output matches all four schema SAMPLEs
exactly and reproduces the $14,391,911 worked example (1,669,906 / 1,029,906 /
640,000 net). E2E test confirms calculator-built tax renders as net with no gross
leakage. Figures remain estimates pending M-706 verification.
