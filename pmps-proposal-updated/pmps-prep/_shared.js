...
/** H2 subsection header — teal, slightly smaller, no border. */
function subSectionHdr(text, color = TEAL, opts = {}) {
  return new Paragraph({
...
function makeHeaderProposal(docType, clientId) {
  return new Header({
    children: [new Paragraph({
      ...
      spacing: { after: 60 },
      children: [
        new TextRun({ text: "AUBREY LAW", font: "Garamond", size: 20, bold: true, color: TEAL }),
 ...
 *   Prepared by
 *   Scott Aubrey, Esq.
 *   Aubrey Law  |  Needham, Massachusetts
...
function titleBlockCover(clientNamesText) {
  return [
    ...
    // Big spacer (firm tagline removed — only "Scott Aubrey, Esq." under "Prepared by")
