...
module.exports = function generateOnePager(data) {
  ...
  let taxTable = null;
  if (s1tax && isCouple) {
    const colHdr2 = hasCreditShelter ? "Credit Shelter Trust" : "Current Plan";
    const rows = [
      new TableRow({ children: [hdrCell("", 4680), hdrCell("No Planning", 2340), hdrCell(colHdr2, 2340)] }),
      new TableRow({ children: [dataCell("Combined Estate", 4680, L_NAVY), dataCell(fmt(s1tax.taxableEstate), 2340, L_NAVY), dataCell(fmt(s1tax.taxableEstate), 2340, L_NAVY)] }),
      new TableRow({ children: [dataCell("MA Estate Tax", 4680), dataCell(fmt(s1tax.netTax), 2340), dataCell(hasCreditShelter && s2tax ? fmt(s2tax.totalNetTax) : fmt(s1tax.netTax), 2340)] }),
    ];
    ...
    const rows = [
      new TableRow({ children: [hdrCell("", 5760), hdrCell("Amount", 3600)] }),
      new TableRow({ children: [dataCell("Taxable Estate", 5760, L_NAVY), dataCell(fmt(s1tax.taxableEstate), 3600, L_NAVY)] }),
      new TableRow({ children: [dataCell("MA Exemption", 5760), dataCell("$2,000,000", 3600)] }),
      new TableRow({ children: [totalCell("MA Estate Tax", 5760), totalCell(fmt(s1tax.netTax), 3600)] }),
    ];
  ...
  const docRows = [new TableRow({ children: docHdr })];
  let rowIdx = 0;
  (data.plan.documents || []).forEach(d => {
    const bg = rowIdx % 2 === 0 ? L_TEAL : GRAY;
    const forLabel = d.joint ? "Joint" : (isCouple && d.perSpouse ? "Each Spouse" : s1.firstName);
  ...
  if (taxTable) {
    ...
    if (data.hasLegalPlan) children.push(bodyP("* Billing estimate. MetLife coverage for tax-planning trusts may require case manager approval.", { italics: true, size: 16, color: "888888" }));
  }
  ...
  if (data.hasLegalPlan) {
    ...
    children.push(bodyP("* MetLife billing estimate. HCP billed as Medical POA; HIPAA as Medical Information POA. Coverage subject to plan terms.", { italics: true, size: 16, color: "888888" }));
  }
  ...
  children.push(new Paragraph({
    children: [new TextRun({ text: `Questions? Contact Scott Aubrey  |  scott@aubreylegal.com  |  1329 Highland Avenue, Suite 1A, Needham, MA 02492`, font: "Garamond", size: 18, color: "888888", italics: true })],
    alignment: AlignmentType.CENTER
  }));
  children.push(spacer(40));
  children.push(bodyP("This summary is for discussion purposes only and does not constitute legal or tax advice. All documents are subject to attorney review before execution.", { italics: true, size: 16, color: "AAAAAA" }));
  ...
  const doc = new Document({
    ...
    sections: [{
      ...
      children: [
        ...titleBlock("ESTATE PLAN SUMMARY", `${names}  |  ${data.plan.type || "Estate Plan"}`, `Aubrey Law  |  ${data.matter.date}`),
        ...children
      ]
    }]
  });
