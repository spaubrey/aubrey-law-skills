...
module.exports = function generateAgenda(data) {
  ...
  if (s1tax) {
    if (hasCreditShelter) {
      ...
      taxNotes.push(`Without a credit shelter trust: at first death, assets pass to survivor (first spouse's $2M exemption is wasted). At second death, the full ${data.assets.grossEstateApprox || data.assets.grossEstateDisplay} is taxed.`);
      taxNotes.push(`Scenario 1 (no planning):  Adjusted taxable estate = ${fmt(s1tax.adjustedTaxableEstate)}  |  MA tax = ${fmt(s1tax.netTax)}`);
      if (s2tax) {
        taxNotes.push(`Scenario 2 (credit shelter):  $2M to bypass trust at first death ($0 tax).  At second death, remaining ${fmt(s2tax.secondDeath ? s2tax.secondDeath.taxableEstate : 0)} is taxed → net tax = ${fmt(s2tax.totalNetTax)}`);
        taxNotes.push(`Tax savings from credit shelter: ${fmt(data.tax.savings)}`);
      }
  ...
  if (data.fiduciaries && data.fiduciaries.notes) {
    fidRows.push(new TableRow({ children: [
      new TableCell({
        ...
        children: [new Paragraph({ children: [new TextRun({ text: "⚑  " + data.fiduciaries.notes, font: "Garamond", size: 18, color: "856404" })] })]
      })
    ]}));
  }
  ...
  if (!fid1.trustee) autoFlags.push("Fiduciary appointments: all unknown — walk through each role at the meeting.");
