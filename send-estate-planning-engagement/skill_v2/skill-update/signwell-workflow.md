  ...
  "files": [
    {
      ...
      "name": "Engagement Agreement - Smith - 2026-05-19.docx"
    }
  ],
  "recipients": [
    {
...
**Option A — URL (legacy SharePoint share-link path):**
...
**Option B — base64 (standard path — use this):**
...
## Note on file entry key

SignWell's `files` array requires the key `name` for the file name —
`file_name` returns 422 "name can't be blank". Both `file_url` and
`file_base64` entries use `name`.
