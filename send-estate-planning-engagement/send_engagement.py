...
def ensure_lead_folder(inputs: dict, today: dt.date | None = None) -> Path:
    ...
    if not ONEDRIVE_ROOT.exists():
        raise SystemExit(
            ...
            "Check that OneDrive is mounted and the folder exists."
        )
...
def signwell_send(
    ...
    file_entry: dict = {"file_name": file_name}
    if file_url:
...
def cmd_prepare(args):
    ...
    final_path = save_to_lead_folder(Path(args.rendered), inputs, today=today)
...
def main():
    ...
    prep.add_argument("--sync-wait", type=int, default=10, help="Seconds to wait for OneDrive sync")
